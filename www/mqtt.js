var mqtt = {
  ...
  init: function(options) {
    ...
    if (mqtt.offlineCaching) {
      mqtt.cache = window.sqlitePlugin.openDatabase(
        { name: "mqttcache.db", location: 2 },
        function(db) {
          db.executeSql(
          ...
          db.transaction(
            function(tx) {
              tx.executeSql(
                "CREATE TABLE IF NOT EXISTS cache(id INTEGER PRIMARY KEY AUTOINCREMENT, topic TEXT, message INT, qos INT, retain INT, sending INT)",
                [],
  ...
  on: function(evt, success, fail) {
    switch (evt) {
      ...
      case "delivered":
        ...
        break;
      case "offline":
        mqtt.onOffline = optional(success, function() {
          console.log("offline");
        });
        break;
    }
  ...
  _connect: function(success, error) {
    cordova.exec(success, error, "MQTTPlugin", "connect", [
      mqtt.host,
      mqtt.port,
      mqtt.options
    ]);
  ...
  publish: function(options, success, fail) {
    ...
    var retain = isset(options.retain) ? options.retain : true;
    ...
    if (mqtt.offlineCaching && mqtt.cache != null) {
      ...
      mqtt.cache.transaction(
        function(tx) {
          tx.executeSql(
            "INSERT INTO cache(topic, message, qos, retain, sending) VALUES(?,?,?,?,?)",
            [topic, message, qos, retain, 0],
  ...
  _publishCached: function(tx) {
    tx.executeSql(
      "SELECT * FROM cache WHERE sending = 0 ORDER BY id",
      [],
      function selectSuccess(tx, res) {
        ...
        for (var i = 0; i < res.rows.length; i++) {
          (function(i, tx) {
            ...
            tx.executeSql(
              "UPDATE cache SET sending = 1 WHERE id = ?",
              [message.id],
                ...
                mqtt._publish(
                      ...
                      function(tx) {
                        ...
                        tx.executeSql(
                          ...
                          function deleteSuccess(tx, res) {
                            console.log(
                              "Deleted Message " + id + " from cache"
                            );
                          },
                          function deleteError(err) {
                            console.error(
                              "Could not delete Message. Error: " + err
                            );
                            return false;
                          }
                        );
                  ...
                  function publishError(result) {
                    ...
                    mqtt.cache.transaction(function(tx) {
                      ...
                      tx.executeSql(
                        "UPDATE cache SET sending = 0 WHERE id = ?",
                        [result.id],
                        function(tx, res) {
  ...
  _publish: function(id, topic, message, qos, retain, success, error) {
    cordova.exec(success, error, "MQTTPlugin", "publish", [
      id,
      topic,
      message,
      qos,
  ...
  showCache: function() {
    mqtt.cache.readTransaction(function(tx) {
      tx.executeSql("SELECT * FROM cache ORDER BY id", [], function(tx, res) {
        console.log("BEGIN");
        for (var i = 0; i < res.rows.length; i++) {
          //(function(i, tx){
          var message = res.rows.item(i);
